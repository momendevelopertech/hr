...
export default function StatsGrid({ stats }: { stats: Stat[] }) {
    ...
    return (
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 px-6">
            {stats.map((stat) => {
                const style = cardStyles[stat.label] || cardStyles.leaveBalance;
                const Icon = style.icon;
                return (
                    <div key={stat.label} className={`card p-4 min-h-[120px] bg-gradient-to-br ${style.shell}`}>
                        <div className="flex items-start justify-between gap-3">
                            <p className="text-xs uppercase tracking-[0.16em] text-ink/60">{t(stat.label as any) ?? stat.label}</p>
                            ...
                            <div className="mt-2.5 space-y-2">
                                {stat.rows.map((row) => (
                                    <div key={row.label} className="flex items-center justify-between gap-3">
                                        <p className="text-[11px] uppercase tracking-[0.06em] text-ink/70">{t(row.label as any) ?? row.label}</p>
                                        <p className={`text-lg font-semibold ${row.valueClassName ?? 'text-ink'}`}>{row.value}</p>
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <p className="mt-2.5 text-xl font-semibold text-ink">{stat.value}</p>
                        )}
                        {stat.hint && <p className="mt-1.5 text-xs text-ink/70">{stat.hint}</p>}
                    </div>
                );
            })}
        </section>
